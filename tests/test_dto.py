...
def test_structured_input():
    class MyInput(StructuredInput):
        ...
        age = Number(minimum=18, integer_only=True)

    MyInput.validate({"name": "rubbie", "age": 21})

    with pytest.raises(ValueError):
        MyInput.validate({"name": "ade", "age": 9})


def test_nested_structured_input():
    class Order(StructuredInput):
        name = String(min_length=2)
        price = Number()

    class MyInput(StructuredInput):
        name = String(min_length=4)
        age = Number(minimum=18, integer_only=False)
        order = Order

    MyInput.validate(
        {"name": "rubbie", "age": 21, "order": {"name": "Coke", "price": 120}}
    )
